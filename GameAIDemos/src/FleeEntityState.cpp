...
GameAIDemos::FleeEntityState::FleeEntityState(LiveEntity* source, PlayerEntity& target) :
	BaseEntityState("Fleeing"), m_sourcePtr(source), m_playerPtr(&target)
{
	enter();
}

void GameAIDemos::FleeEntityState::enter()
{
	m_sourcePtr->setSpriteColor(sf::Color::Red);
}

void GameAIDemos::FleeEntityState::update()
{
	...
	// Our direction
	sf::Vector2f direction = sourcePos - targetPos;
...
void GameAIDemos::FleeEntityState::exit()
{
	m_sourcePtr->setVelocity(sf::Vector2f(0.0f, 0.0f));
}

GameAIDemos::BaseEntityState* GameAIDemos::FleeEntityState::getNewState()
{
	return nullptr;
}
